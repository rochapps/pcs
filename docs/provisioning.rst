...
Pcs is deployed on the following stack.
...
documented on that page, you need to create a new file in ``pcs/celery.py`` and then
modify ``pcs/__init__.py`` to import that file. You'll also need to customize ``{{
...
Additionally you will need to uncomment the ``BROKER_URL`` setting in the project settings::
    ...
    # pcs/settings/deploy.py
    from .base import *

    # ...
    BROKER_URL = 'amqp://pcs_%(ENVIRONMENT)s:%(BROKER_PASSWORD)s@%(BROKER_HOST)s/pcs_%(ENVIRONMENT)s' % os.environ
...
the broker by default. The RabbitMQ user will be named ``pcs_<environment>`` and the
vhost will be named ``pcs_<environment>`` for each environment. It requires that you
...
You can use the below OpenSSL commands to generate the key and signing request::
  ...
  openssl genrsa -out pcs.key 2048
  # Make copy of the key with the passphrase
  cp pcs.key pcs.key.secure
  # Remove any passphrase
  openssl rsa -in pcs.secure -out pcs.key
  # Generate signing request
  openssl req -nodes -sha256 -new -key pcs.key -out pcs.csr
